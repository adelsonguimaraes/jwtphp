...
$key = "@123"; // alterar para key única e secreta
$time = 1200; // tempo em segundos para expiração do token, recomendado 1200s

function generate ($dados) {
    global $key;
...
function validate() {
    ...
    if (!empty($http_header['Authorization'])) {
        ...
        $data_exp = (new DateTime())->setTimestamp($payload_decode->exp)->add(new DateInterval("PT{$time}S"))->getTimestamp();//format('Y-m-d H:i:s');
